...
const buildQuery = (data, ID, KEY) => {
  ...
  for(let i=0;i<count;i++){
    let str = labelArr[i];
    let query = `q=${str}&app_id=${ID}&app_key=${KEY}&to=${type}&diet=${ data.diet!=="weight-loss" && data.diet!=="weight-gain" ? data.diet : "balanced"}${health}&calories=${calories.min}-${calories.max}`;
    queries[str]= query;
  }
  return queries;
}
